        ...
        <div id="plist" class="row bg-white ml-1 mr-1 pt-2 pl-2">
          <div v-for="(p,i) of products" :key="i" class="col-md-4 p-1">
            <div class="card mb-4 box-shadow pr-2 pl-2">
              <router-link :to="`/products/${p.lid}`">
                <img class="card-img-top" :src="p.md">
              </router-link>
              <div class="card-body p-0">
                <h5 class="text-primary">￥{{p.price.toFixed(2)}}</h5>
                <p class="card-text">
                  <router-link :to="`/products/${p.lid}`" class="text-muted small" :title="p.title" v-text="p.title"></router-link>>
                </p>
        ...
        <h6 class="mb-3 p-2 text-muted small">
          <nav aria-label="Page navigation example">
            <ul class="pagination mb-0 justify-content-end">
              ...
              <li v-for="i of pcount" :key="i" class="page-item"><a class="page-link bg-transparent" href="#" v-text="i"></a></li>
...
export default {
    ...
    props:["kw"],
    methods:{
        load(){//无论做什么前端，都要先封装调用服务端接口获取数据的函数
            this.axios.get(
                "/products",
                {
                    params:{
